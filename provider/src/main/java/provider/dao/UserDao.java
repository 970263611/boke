...
public interface UserDao {
     ...
     * select2加载数据
     */
    @Select("select typeId id,typeName text from article_type where createUser = '0' or createUser = #{id}")
    List<Map> getArticleTypes(@Param("id") String id);

    /**
     * 文章加载数据匹配
     */
    @Select("select typeId id,typeName text from article_type")
    List<Map> getArticleTypeNoCheck();
}
